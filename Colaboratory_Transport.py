...
class Colaboratory_Transport():
  ...
  def solve(self, T=[]):
    ...
    if T == []:
      ...
      for i in I:
        for j in I:
          m.addConstr((N_i[i]+N_i[j])*X[i, j] <= self.cap, name='Rest7')

      m.addConstr(
          quicksum(N_i[i]*X[i, 'c'] for i in I_c) + 
          quicksum(N_i[i]*X[i, j] for i in I_c for j in I_c) == self.dem, name='Rest8')
      ...
      for t in T:
        m.addConstr(
            quicksum(N_i[i]*X[i, 'c', t] for i in I_c) + 
            quicksum(N_i[i]*X[i, j, t] for i in I_c for j in I_c) == self.dem, name='Rest8')
